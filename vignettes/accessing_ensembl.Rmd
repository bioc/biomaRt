...
If we print the current `ensembl` object, we can see that the `ENSEMBL_MART_ENSEMBL` database ^[this is how Ensembl name the database on their server] has been selected, but that no dataset has been chosen.
...
```{r ensembl4, eval = TRUE}
ensembl <- useEnsembl(biomart = "genes", dataset = "hsapiens_gene_ensembl")
```

## Ensembl site

The Ensembl mirror sites are deprecated. `biomaRt` now uses the primary
`www.ensembl.org` site. The `mirror` argument is retained for compatibility
and accepts only `www`.
...
attempt to find the correct BioMart dataset ^[We get a warning for the term 'mouse' as there are multiple strains in BioMart.  In this case **biomaRt** selects the reference strain.]  Our returned table contains the paired Ensembl IDs between the two organisms.
...
If you are unable to modify the `r BiocStyle::Biocpkg("biomaRt")` code (for example if you are using another package that calls `r BiocStyle::Biocpkg("biomaRt")` as part of one if its functions) it's still possible to modify the connection settings for your R session. Below are some reported error messages and code that has been known to resolve them.  You will need to execute this code only once in an R session.  The settings will then be saved to the `r BiocStyle::Biocpkg("biomaRt")` cache and will be used from then on when you try to connect to the Ensembl BioMart.

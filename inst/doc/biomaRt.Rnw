...
In this task we start out with a list of EntrezGene identiers and we want to retrieve GO terms that are associated with these identifiers.  Again we look at the output of \Rfunction{listAttributes} and \Rfunction{listFilters}  to find the filter and attributes we need.  Then we construct the following query:

\begin{scriptsize}
<<>>=
entrez=c("673","837")
getBM(attributes=c('entrezgene','go_biological_process_id','go_biological_process_linkage_type'), filters='entrezgene', values=entrez, mart=ensembl)
@ 
\end{scriptsize}
...
\subsubsection{getSNP}
\Rfunction{getSNP} is a wrapper function for retrieving SNP data given a region on the genome.
\begin{small}
<<echo=TRUE, eval=FALSE>>=
...
\subsubsection{getHomolog}
The \Rfunction{getHomolog} is a wrapper function for mapping identifiers from one species to another.  As described above this can also be done with the more general \Rfunction{getLDS} function.
...
It is possible to query archived versions of Ensembl through \Rpackage{biomaRt}.  The steps below show how to do this.

...
ex_mart <- Mart(
  ...
  filters = data.frame(
    ...
    description = c(
      "Ensembl ID",
      "HGNC Symbol",
      "Gene seq including exons and introns"
    ),
...
test_that("Ensembl URLs are constructed correctly", {
  ...
  expect_error(
    .constructEnsemblURL(version = "00"),
...
test_that("correct settings are applied for getSequence using coordinates.", {
  ...
  expect_true(.getSequenceFromCoords(
    chromosome = 1,
    start = 1,
    end = 2,
    type = "ensembl_gene_id",

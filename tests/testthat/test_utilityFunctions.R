...
bad_result <- data.frame(
  "Not a real column" = 1:2,
  "Ensembl Gene ID" = 3:4,
  check.names = FALSE
)
good_result <- data.frame(
  "Chromosome/scaffold name" = 1:2,
  "Gene stable ID" = 3:4,
  check.names = FALSE
)
...
<tr>
  ...
  expect_silent(
    res_html <- .fetchHTMLresults(host, query, http_config = http_config)
  )

  expect_identical(
    res_html,
    data.frame(
      "Gene stable ID" = "ENSG00000100036",
      "HGNC symbol" = "SLC35E4",
      check.names = FALSE
    )
  )
...
test_that("we can search predefined filter values", {
  expect_equal(
    length(searchFilterOptions(ensembl, filter = "chromosome_name")),
    5
  )

  expect_equal(
    length(searchFilterOptions(ensembl, "chromosome_name", "^[0-9]*$")),
    4
  )

  expect_equal(
    searchFilterOptions(ensembl, "chromosome_name", "PATCH"),
    "CHR_HG1_PATCH"
  )
})
